...
namespace HospitalAnaCosta.Framework.Collections.Generics
{
    ...
    {
        public static Predicate<T> Build<T>(string Expression, bool bStringIsCaseSensitive)
        {
            return _Build<T>(Expression, bStringIsCaseSensitive);
        }

        public static Predicate<T> Build<T>(string Expression)
        {
            return _Build<T>(Expression, true);
        }

        private static Predicate<T> _Build<T>(string Expression, bool bStringIsCaseSensitive)
        {
            Type type = typeof(T);
            ...
            if (bString)
            {
                if (bStringIsCaseSensitive)
                    rhs = string.Format("\"{0}\"", rhs);
                else
                    rhs = string.Format("\"{0}\"", rhs.ToLower());
            }
            ...
            string code;
            if (bStringIsCaseSensitive)
                code = "using System; using System.Collections.Generic; namespace DynAsm { public class Match { public static bool MatchFunc(" + type.FullName + " f) { return f." + lhs + " " + operAtor + " " + rhs + "; } public static Predicate<" + type.FullName + "> MatchPred = new Predicate<" + type.FullName + ">(Match.MatchFunc); public static Predicate<" + type.FullName + "> GetPredicate() { return Match.MatchPred; } } }";
            else
                code = "using System; using System.Collections.Generic; namespace DynAsm { public class Match { public static bool MatchFunc(" + type.FullName + " f) { return f." + lhs + ".ToLower() " + operAtor + " " + rhs + "; } public static Predicate<" + type.FullName + "> MatchPred = new Predicate<" + type.FullName + ">(Match.MatchFunc); public static Predicate<" + type.FullName + "> GetPredicate() { return Match.MatchPred; } } }";
