CREATE OR REPLACE PROCEDURE PRC_CAD_MTMD_INV_ATIVAR_GRUPO
(
     ...
     pCAD_MTMD_GRUPO_ID IN TB_CAD_MTMD_INVENTARIO_FECHA.CAD_MTMD_GRUPO_ID%type
)
is
vDT_INICIO_INV DATE;
vDT_FECHA_INV  DATE;
vFL_MEDICAMENTO  TB_CAD_MTMD_INVENTARIO_FECHA.FL_MEDICAMENTO%type;
/********************************************************************
*    Procedure: PRC_CAD_MTMD_INVENTARIO_ATIVAR
*
*    Data Criacao:  10/22   Por: Andre
*
*    Funcao: Ativa inventario por Grupo
*******************************************************************/
begin
  IF (pCAD_MTMD_ANDAMENTO IN (1,2)) THEN
    -- SE TIVER INVENTARIO EM ANDAMENTO DO SETOR ABORTAR ATIVAC?O
    IF (FNC_MTMD_SETOR_INVENTARIADO(pCAD_SET_ID, pCAD_MTMD_FILIAL_ID, 1, pCAD_MTMD_GRUPO_ID) IN (1,2)) THEN
       RAISE_APPLICATION_ERROR(-20010,'NAO FOI POSSIVEL ATIVAR O INVENTARIO DESTE GRUPO DESTE SETOR NESTA DATA, POIS JA TEM UM EM ANDAMENTO');
    END IF;
    vDT_INICIO_INV := SYSDATE;
  ELSE
    vDT_FECHA_INV := SYSDATE;
  END IF;
  IF (pCAD_MTMD_GRUPO_ID = 1) THEN
    vFL_MEDICAMENTO := 1;
  ELSE
    vFL_MEDICAMENTO := 0;
  END IF;
  ...
  IF (pCAD_MTMD_ANDAMENTO IN (1,2)) THEN
    FOR ONLINE IN ( SELECT MTMD.CAD_MTMD_ID, ESTLOC.MTMD_ESTLOC_QTDE
                    ...
                    AND   MTMD.CAD_MTMD_GRUPO_ID              = pCAD_MTMD_GRUPO_ID
    ...
    LOOP
      ...
      END;
    END LOOP;
    IF (NVL(pCAD_MTMD_GRUPO_ID,0) = 1) THEN
    ...
    END IF;
  END IF;
end PRC_CAD_MTMD_INV_ATIVAR_GRUPO;