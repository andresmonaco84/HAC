CREATE OR REPLACE PROCEDURE PRC_COB_REL_05_ISS_N_RETIDO
  (
  ...
  is
  /********************************************************************
  *    Procedure: PRC_COB_REL_05_ISS_N_RETIDO
  *
  *    Data Criação: 24/10/2012  Por: PEDRO
  *    Alteracao:
  *
  * UTILIZADO NOS RELATORIOS 05
  *******************************************************************/
...
SELECT DISTINCT
       ...
       NVL(SUM(MGC.COB_MGC_VL_ISS) OVER(PARTITION BY NOF.FAT_NOF_ID,TRUNC(MGC.COB_MGC_DT_MOVIMENTO)),0) COB_MGC_VL_ISS,
       NVL(SUM(MGC.COB_MGC_VL_IR) OVER(PARTITION BY NOF.FAT_NOF_ID,TRUNC(MGC.COB_MGC_DT_MOVIMENTO)),0) COB_MGC_VL_IR,
       NVL(SUM(MGC.COB_MGC_VL_CSLL) OVER(PARTITION BY NOF.FAT_NOF_ID,TRUNC(MGC.COB_MGC_DT_MOVIMENTO)),0) COB_MGC_VL_CSLL,
       NVL(SUM(MGC.COB_MGC_VL_PIS) OVER(PARTITION BY NOF.FAT_NOF_ID,TRUNC(MGC.COB_MGC_DT_MOVIMENTO)),0) COB_MGC_VL_PIS,
       NVL(SUM(MGC.COB_MGC_VL_COFINS) OVER(PARTITION BY NOF.FAT_NOF_ID,TRUNC(MGC.COB_MGC_DT_MOVIMENTO)),0) COB_MGC_VL_COFINS,
       
       SUM(MGC.COB_MGC_VL_MOVIMENTO) OVER(PARTITION BY NOF.FAT_NOF_ID,TRUNC(MGC.COB_MGC_DT_MOVIMENTO)) COB_MGC_VL_MOVIMENTO,

       SUM(NVL(MGC.COB_MGC_VL_MOVIMENTO,0) + NVL(MGC.COB_MGC_VL_ISS,0) + NVL(MGC.COB_MGC_VL_IR,0) + NVL(MGC.COB_MGC_VL_CSLL,0) +
           NVL(MGC.COB_MGC_VL_PIS,0) + NVL(MGC.COB_MGC_VL_COFINS,0)) OVER(PARTITION BY NOF.FAT_NOF_ID,TRUNC(MGC.COB_MGC_DT_MOVIMENTO)) VALOR_DIGITADO,
            --vl calculado na tela
       SUM(NVL(MGC.COB_MGC_VL_MOVIMENTO,0) + NVL(MGC.COB_MGC_VL_ISS,0) + NVL(MGC.COB_MGC_VL_IR,0) + NVL(MGC.COB_MGC_VL_CSLL,0) +
           NVL(MGC.COB_MGC_VL_PIS,0) + NVL(MGC.COB_MGC_VL_COFINS,0)) OVER(PARTITION BY NOF.FAT_NOF_ID,TRUNC(MGC.COB_MGC_DT_MOVIMENTO))
             -  NVL(SUM(MGC.COB_MGC_VL_MOVIMENTO) OVER(PARTITION BY NOF.FAT_NOF_ID,TRUNC(MGC.COB_MGC_DT_MOVIMENTO)),0) TOTAL_RETENCOES
...
AND (
  ...
  ;
             io_cursor := v_cursor;
  end PRC_COB_REL_05_ISS_N_RETIDO;
