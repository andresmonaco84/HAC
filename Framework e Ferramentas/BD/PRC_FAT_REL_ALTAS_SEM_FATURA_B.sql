CREATE OR REPLACE PROCEDURE "PRC_FAT_REL_ALTAS_SEM_FATURA_B"
(
...
IS
/********************************************************************
*    PROCEDURE: PRC_FAT_REL_ALTAS_SEM_FATURA_B
*
*    DATA ALTERACAO: 16/05/2013 POR: Pedro
*    ALTERAC?O: Limpeza
...
v_cursor PKG_CURSOR.t_cursor;
 ...
 V_WHERE_CCP  varchar2(1000);
  ...
  V_WHERE_INA := NULL;
   ...
   -- LEFT JOIN TB_CAD_MPF_MOTI_PEND_FATURAR MPF  ON      MPF.CAD_MPF_ID                = CCI.CAD_MPF_ID
...
AND     (CCI.FAT_CCI_FL_PACOTE = '||chr(39)||'N'||chr(39)||' OR CCI.FAT_CCI_FL_PACOTE IS NULL)
   ...
   AND    (CCI.ATD_ATE_TP_PACIENTE IN (' ||chr(39)|| 'A' ||chr(39)|| ',' ||chr(39)|| 'U' ||chr(39)|| ',' ||chr(39)|| 'E' ||chr(39)|| ') AND (((TRUNC(SYSDATE) - CCI.FAT_CCI_DT_INICIO_CONSUMO) >= CNV.CAD_CNV_QT_DIA_REALIZ_FATUR)
          )
       OR (CCI.ATD_ATE_TP_PACIENTE = ' ||chr(39)|| 'I' ||chr(39)|| ' AND ((TRUNC(SYSDATE) - CCI.FAT_CCI_DT_INICIO_CONSUMO) >= CNV.CAD_CNV_QT_DIA_REALIZ_FATUR)  AND (((TRUNC(SYSDATE) - CCP.FAT_CCP_DT_PARCELA) >= CNV.CAD_CNV_QT_DIA_REALIZ_FATUR) OR CCP.FAT_CCP_DT_PARCELA IS NULL) )
          )
   --AND    ((CCP.FAT_CCP_FL_STATUS   = ' ||chr(39)|| 'A' ||chr(39)|| ' AND CCP.FAT_NOF_ID IS NULL) OR (CCI.FAT_CCP_ID IS NULL))
...
' || V_WHERE || '
   ...
   AND     (--(pFATURADO IS NOT NULL AND CCP.FAT_NOF_ID IS NOT NULL )
        (' ||chr(39)|| pAUDITORIA ||chr(39)|| ' IS NOT NULL AND CCP.FAT_CCP_FL_STATUS_AUDIT = ' ||chr(39)|| 'E' ||chr(39)|| ')
    ...
    GROUP BY
    DECODE(CCI.ATD_ATE_TP_PACIENTE,' ||chr(39)|| 'A' ||chr(39)|| ',' ||chr(39)|| 'AMBULATORIO' ||chr(39)|| ',' || chr(39) || 'U' || chr(39) || ',' || chr(39) || 'URGENCIA' || chr(39) || ',' || chr(39) || 'I' || chr(39) || ',' || chr(39) || 'INTERNADO' || chr(39) || ',' || chr(39) || 'E' || chr(39) || ',' || chr(39) || 'EXTERNO' || chr(39) || ') ,
            CCI.ATD_ATE_TP_PACIENTE,
            CNV.CAD_CNV_CD_HAC_PRESTADOR,
            CNV.CAD_CNV_NM_FANTASIA,
            PLA.CAD_PLA_CD_PLANO_HAC,
        ...
        --    MPF.CAD_MPF_DS_MOTI_PEND_FATURAR,
            CNV.CAD_CNV_QT_DIA_REALIZ_FATUR,
            TCO.FAT_TCO_DS_COMANDA,
            setor.cad_set_ds_setor,ate.atd_ate_id

    ORDER   BY  CNV.CAD_CNV_CD_HAC_PRESTADOR,CCI.ATD_ATE_TP_PACIENTE,ATE.ATD_ATE_DT_ATENDIMENTO,ate.atd_ate_id DESC';
   -- TESTE :=  V_SELECT ;
OPEN v_cursor FOR
     V_SELECT ;
   --SELECT 1 FROM DUAL;
    io_cursor := v_cursor;
END PRC_FAT_REL_ALTAS_SEM_FATURA_B;
