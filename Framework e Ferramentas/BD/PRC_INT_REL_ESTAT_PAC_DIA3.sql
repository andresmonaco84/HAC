create or replace procedure prc_int_rel_estat_pac_dia3
  (
...
begin
  OPEN v_cursor FOR
   SELECT  DISTINCT     SETOR.CAD_SET_DS_SETOR,
              ...
              ''  MES_1,
              ''  MES_2,
              to_char(ADD_MONTHS(pATD_ATE_DT_ATENDIMENTO_FIM,-2),'MM/yyyy')  MES_3,
  ...
  LEFT JOIN  ( SELECT
               ...
               to_char(ADD_MONTHS(pATD_ATE_DT_ATENDIMENTO_FIM,-2),'MM/yyyy') MES,
               (to_date(TO_CHAR(LAST_DAY(add_months(pATD_ATE_DT_ATENDIMENTO_FIM,-2)),'DD')||to_char(add_months(pATD_ATE_DT_ATENDIMENTO_FIM,-2),'mm/yyyy'),'dd/mm/yyyy')
                - to_date( '01/'||to_char(add_months(pATD_ATE_DT_ATENDIMENTO_FIM,-2), 'mm/yyyy'),'dd/mm/yyyy')+1)
                 * (SELECT COUNT(DECODE(QLE.CAD_QLE_TP_QUARTO_LEITO,'I','I')) FROM  TB_CAD_QLE_QUARTO_LEITO QLE WHERE
                 ...
                 ON   LAT.CAD_LAT_ID_LOCAL_ATENDIMENTO = SETOR.CAD_LAT_ID_LOCAL_ATENDIMENTO
                 WHERE
                       (to_char(ICD.ATD_ICD_DATA_CENSO,'MM/yyyy') = to_char(ADD_MONTHS(pATD_ATE_DT_ATENDIMENTO_FIM,-2),'MM/yyyy'))
 ...
 --and NVL(CENSO.PCT_OCUPACAO,0) > 0
  ...
  ;
  io_cursor := v_cursor;
end PRC_INT_REL_ESTAT_PAC_DIA3;
/
