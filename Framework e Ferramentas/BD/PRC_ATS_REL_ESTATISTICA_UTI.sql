CREATE OR REPLACE PROCEDURE "PRC_ATS_REL_ESTATISTICA_UTI"
  (
  ...
  is
  /********************************************************************
  *    Procedure: PRC_ATS_REL_ESTATISTICA_UTI
  *
...
OPEN v_cursor FOR
       ...
       case when SETOR_PROC.CAD_SET_DS_SETOR IN ('UTI INFANTIL','UTI NEONATAL') then 'UTI INFANTIL'
       WHEN SETOR_PROC.CAD_SET_DS_SETOR = ('UTI GERAL') then 'UTI GERAL'
       WHEN SETOR_PROC.CAD_SET_DS_SETOR = ('UTI CARDIOLOGICA') then 'UTI CARDIOLOGICA'
       END ATS_ATE_IN_INTLIB,
       ...
       WHEN pla.cad_pla_cd_tipoplano = 'SP' THEN 'S.PRESTADO'
       ELSE 'PARTICULAR'
       end || case when SETOR_PROC.CAD_SET_DS_SETOR IN ('UTI INFANTIL','UTI NEONATAL') then 'UTI INFANTIL'
       WHEN SETOR_PROC.CAD_SET_DS_SETOR = 'UTI GERAL' then 'UTI GERAL'
       WHEN SETOR_PROC.CAD_SET_DS_SETOR = 'UTI CARDIOLOGICA' then 'UTI CARDIOLOGICA'
       END) TOTAL_PERIODO,
       ...
       (SELECT  SUM( Count(ATS.ATS_ATE_CD_INTLIB))     OVER()     GERAL
         FROM TB_ATS_ATE_ATENDIMENTO_SADT ATS
               ...
               LEFT JOIN TB_CAD_SET_SETOR SETOR_PROC
               ON SETOR_PROC.CAD_SET_ID = ATS.CAD_SET_ID
          ...
          WHERE   ATS.ATS_ATE_DT_REALIZ_PROCED BETWEEN pDT_INI_CONSULTA AND pDT_FIM_CONSULTA
                 ...
                 AND (pCAD_LAT_ID_LOCAL_ATENDIMENTO IS NULL OR UNI.CAD_UNI_ID_UNIDADE = pCAD_LAT_ID_LOCAL_ATENDIMENTO)
       ...
       Round((SUM( Count(ATS.ATS_ATE_CD_INTLIB))
       OVER(PARTITION BY case when pla.cad_pla_cd_tipoplano in ('GB','PL') then 'GLOBAL + PSAC'
       WHEN PLA.CAD_PLA_CD_TIPOPLANO = 'FU' THEN 'FUNCIONARIO HAC'
       WHEN pla.cad_pla_cd_tipoplano = 'SP' THEN 'S.PRESTADO'
       ELSE 'PARTICULAR'
       ...
       WHEN SETOR_PROC.CAD_SET_DS_SETOR = 'UTI GERAL' then 'UTI GERAL'
       WHEN SETOR_PROC.CAD_SET_DS_SETOR = 'UTI CARDIOLOGICA' then 'UTI CARDIOLOGICA'
       END) *100) / (SELECT  SUM( Count(ATS.ATS_ATE_CD_INTLIB))   OVER()       GERAL
                           FROM TB_ATS_ATE_ATENDIMENTO_SADT ATS
                                 LEFT JOIN TB_ATS_APL_ATEN_PROCED_LAUDO APL
                                 ON APL.ATS_ATE_ID = ATS.ATS_ATE_ID
                                 AND APL.ATS_ATE_CD_INTLIB = ATS.ATS_ATE_CD_INTLIB
       ...
       ,2) PERCENTUAL
          ...
          WHERE   ATS.ATS_ATE_DT_REALIZ_PROCED BETWEEN pDT_INI_CONSULTA AND pDT_FIM_CONSULTA
               ...
               AND (ATS.ATS_ATE_FL_STATUS <> 'C')
               AND (ATS.ATS_ATE_IN_INTLIB <> 'L')
               and (SETOR_PROC.CAD_SET_DS_SETOR IN ('UTI INFANTIL','UTI NEONATAL','UTI GERAL','UTI CARDIOLOGICA'))
     ...
     io_cursor := v_cursor;
  end PRC_ATS_REL_ESTATISTICA_UTI;
