create or replace procedure prc_int_rel_estat_pac_dia
  (
...
begin
  ...
  SELECT  DISTINCT     SETOR.CAD_SET_DS_SETOR,
              ...
              to_char(pATD_ATE_DT_ATENDIMENTO_FIM,'MM/yyyy')  MES_1,
              ''  MES_2,
              ''  MES_3,
              ...
              NVL(sum(CENSO.ATD_IAC_QT_PACIENTE_DIA),0)   TOTAL_ATD_IAC_QT_PACIENTE_DIA
  ...
  LEFT JOIN  ( SELECT
               ...
               to_char(pATD_ATE_DT_ATENDIMENTO_FIM,'MM/yyyy') MES,
               (pATD_ATE_DT_ATENDIMENTO_FIM - to_date( '01/'||to_char(sysdate, 'mm/yyyy'),'dd/mm/yyyy')+1)
                 * (SELECT COUNT(DECODE(QLE.CAD_QLE_TP_QUARTO_LEITO,'I','I')) FROM  TB_CAD_QLE_QUARTO_LEITO QLE WHERE
                   ...
                   (SELECT COUNT(DECODE(QLE.CAD_QLE_TP_QUARTO_LEITO,'I','I'))
                     ...
                     CASE WHEN ((pATD_ATE_DT_ATENDIMENTO_FIM -  to_date( '01/'||to_char(pATD_ATE_DT_ATENDIMENTO_FIM, 'mm/yyyy'),'dd/mm/yyyy')+1) *
                             (SELECT COUNT(DECODE(QLE.CAD_QLE_TP_QUARTO_LEITO,'I','I')) FROM  TB_CAD_QLE_QUARTO_LEITO QLE WHERE
                                                                                               QLE.CAD_SET_ID = SETOR.CAD_SET_ID))
                             > 0 THEN
                             (
                  NVL(ROUND((SUM(ICD.ATD_IAC_QT_PACIENTE_DIA) OVER(PARTITION BY SETOR.CAD_SET_ID||TO_CHAR(ICD.ATD_ICD_DATA_CENSO,'MM/yyyy'))) * 100
                              /
                             ((pATD_ATE_DT_ATENDIMENTO_FIM -  to_date( '01/'||to_char(pATD_ATE_DT_ATENDIMENTO_FIM, 'mm/yyyy'),'dd/mm/yyyy')+1) *
                             (SELECT COUNT(DECODE(QLE.CAD_QLE_TP_QUARTO_LEITO,'I','I')) FROM  TB_CAD_QLE_QUARTO_LEITO QLE WHERE
                                                                                               QLE.CAD_SET_ID = SETOR.CAD_SET_ID)),2),0)
                             ) END PCT_OCUPACAO, ---------------
                    NVL(round(
                      CASE WHEN
                      (
                ...
                (  NVL((SUM(ICD.ATD_ICD_QT_SAIDA)
                 ...
                 ON   LAT.CAD_LAT_ID_LOCAL_ATENDIMENTO = SETOR.CAD_LAT_ID_LOCAL_ATENDIMENTO
                 WHERE
                       to_char(ICD.ATD_ICD_DATA_CENSO,'MM/yyyy') = to_char(pATD_ATE_DT_ATENDIMENTO_FIM,'MM/yyyy')
