create or replace procedure PRC_INT_REL_RESUMO_UTI
  (
...
--pCAD_CGC_ID_E IN TB_CAD_CNV_CONVENIO.CAD_CGC_ID%TYPE DEFAULT NULL,
  ...
  /********************************************************************
  *    Procedure: PRC_INT_REL_RESUMO_UTI
  *
  *    Data Criacao:  11/09/2009   Por: pedro
  *    Data Alteracao:  04/12/2013  Por: Cristiane
  *    Alterac?o: Desconsiderar leitos extras e desativados
  *
...
begin
  OPEN v_cursor FOR
        SELECT distinct
            ...
            --   atd.atd_ate_id,
                ...
                case when cnv.cad_cnv_cd_hac_prestador = 'SD01' then 'ACS'
                    ...
                    when cnv.cad_cgc_id = 2 AND cnv.cad_cnv_id_convenio!=282 then 'MERCADO' 
                    WHEN cnv.cad_cgc_id = 2 AND cnv.cad_cnv_id_convenio =282 THEN 'PARTIC.'
                    ELSE '' end subgrupo ,
                COUNT(DISTINCT ATD.ATD_ATE_ID) OVER (PARTITION BY SETOR.CAD_SET_ID||case when cnv.cad_cnv_cd_hac_prestador = 'SD01' then 'ACS'
                    when cnv.cad_cgc_id = 1 and cnv.cad_tpe_cd_codigo = 'SP' and cnv.cad_cnv_cd_hac_prestador != 'S077' then 'AMIL'
                    when cnv.cad_cnv_cd_hac_prestador in ('GG05', 'HAC', 'NP01', 'NR14', 'S077') then 'FUNC.'
                    when cnv.cad_cgc_id = 2 AND cnv.cad_cnv_id_convenio!=282 then 'MERCADO' 
                    WHEN cnv.cad_cgc_id = 2 AND cnv.cad_cnv_id_convenio =282 THEN 'PARTIC.'
                ...
                ON       SETOR.CAD_SET_ID       = QLE.CAD_SET_ID
                        AND SETOR.CAD_CSE_ID NOT IN (8,9)--AND SETOR.CAD_SET_ID NOT IN (2072,2312,5,140,2472)
        ...
        LEFT JOIN     TB_CAD_CID_CID10                   CID_IEP        ON            CID_IEP.CAD_CID_CD_CID10         = IEP.CAD_CID_CD_CID10
         ...
         AND  SETOR.CAD_SET_DS_SETOR IN ('UTI GERAL','UTI CARDIOLOGICA','UTI INFANTIL','UTI NEONATAL')
