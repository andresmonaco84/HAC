CREATE OR REPLACE PROCEDURE PRC_REP_IMP_PRODUTIVIDADE_P8(pFAT_CCI_DT_INICIO_CONSUMO_INI IN TB_FAT_CCI_CONTA_CONSU_ITEM.FAT_CCI_DT_INICIO_CONSUMO%TYPE,
...
BEGIN
    ...
    CURSOR FAT IS
      SELECT DISTINCT ATD.ATD_ATE_ID,
       ...
       WHERE CCI.FAT_CCI_CD_CLINICA_PROF IN
         ...
         AND CCI.CAD_UNI_ID_UNIDADE IS NULL
         ...
         AND TRUNC(CCI.FAT_CCI_DT_INICIO_CONSUMO) BETWEEN
             TO_DATE(pFAT_CCI_DT_INICIO_CONSUMO_INI, 'DD/MM/YYY') AND
  ...
  BEGIN
    ...
    COMMIT;
  END;
END PRC_REP_IMP_PRODUTIVIDADE_P8;
/
