create or replace procedure PRC_FAT_REL_OPM_UTIL_INT
  (
  ...
  is
  /********************************************************************
  *    Procedure: PRC_FAT_REL_OPM_UTIL_INT
  *
...
select  S.NR_ATENDIMENTO,
      ...
      O.NR_SEQ SEQUENCIALOPM,
             O.CD_TABELA_OPM TABELAOPM,
             O.CD_OPM CODIGODOOPM,
             O.DS_OPM DESCRICAOOPM,
             REPLACE(TO_CHAR(O.QT_OPM),'.', ',') QTDOPM,
             O.VL_UNIT_OPM VALORUNIOPM,
             O.VL_TOTAL_OPM VALORTOTALOPM,
             O.CD_BARRAS_OPM CODIGOBARRASOPM  

    from     TSS_RESUMO_INT_SGS S,
             TSS_RESUMO_INT_OPM_SGS  O ,
             TB_FAT_FCL_CONTR_EMI_LOTE FCL

    where

        S.NR_ATENDIMENTO      = O.NR_ATENDIMENTO--(+)
