create or replace PROCEDURE PRC_INT_REL_SAIDA_SEM_PRINCP
...
io_cursor                     OUT PKG_CURSOR.t_cursor
  ...
  /********************************************************************
  *    Procedure: PRC_INT_REL_SAIDA_SEM_PRINCP
  *
  *    data: 18/02/2013
  *    Alterai??i??o:  select inst.ger,
  *
  *
  *******************************************************************/
...
IF pATD_ATE_HR_ATENDIMENTO_INI IS NOT NULL THEN V_WHERE := V_WHERE || ' AND INA.ATD_INA_HR_ALTA_CLINICA >= ' || pATD_ATE_HR_ATENDIMENTO_INI; END IF;
IF pATD_ATE_HR_ATENDIMENTO_FIM IS NOT NULL THEN V_WHERE := V_WHERE || ' AND INA.ATD_INA_HR_ALTA_CLINICA <= ' || pATD_ATE_HR_ATENDIMENTO_FIM; END IF;
...
IF pATD_ATE_DT_ATENDIMENTO_FIM IS NOT NULL THEN V_WHERE := V_WHERE || ' AND INA.ATD_INA_DT_ALTA_ADM <= ' ||CHR(39)|| pATD_ATE_DT_ATENDIMENTO_FIM ||CHR(39); END IF;
  ...
  V_SELECT :=
   '
   SELECT DISTINCT IML_QLE.ATD_IML_DT_ENTRADA,
                    IML_QLE.ATD_IML_HR_ENTRADA,
                    DECODE(IML_QLE.CAD_QLE_TP_QUARTO_LEITO,'||CHR(39)||'I'||CHR(39)||','||CHR(39)||'INTERNO'||CHR(39)||','||CHR(39)||'E'||CHR(39)||','||CHR(39)||'EXTRA'||CHR(39)||') CAD_QLE_TP_QUARTO_LEITO,
                    IML_QLE.CAD_QLE_NR_QUARTO,
                    IML_QLE.CAD_QLE_NR_LEITO,
                    CASE WHEN AIC.ATD_AIC_TP_SITUACAO_PAC = '||CHR(39)||'A'||CHR(39)||' AND INA.ATD_INA_DT_ALTA_ADM IS NOT NULL THEN nvl(Round(INA.ATD_INA_DT_ALTA_ADM - ATD.ATD_ATE_DT_ATENDIMENTO),0)
                         WHEN AIC.ATD_AIC_TP_SITUACAO_PAC = '||CHR(39)||'I'||CHR(39)||' THEN nvl(Round(sysdate - ATD.ATD_ATE_DT_ATENDIMENTO), 0)
                         ELSE  0
                    END PERMANENCIA,
                    ...
                    DECODE(AIC.ATD_AIC_TP_SITUACAO_PAC,'||CHR(39)||'I'||CHR(39)||','||CHR(39)||'INTERNADO'||CHR(39)||','||CHR(39)||'A'||CHR(39)||','||CHR(39)||'ALTA'||CHR(39)||') ATD_AIC_TP_SITUACAO_PAC,
      ...
      LEFT JOIN (SELECT BNF_SIT.DESSITBEN,
                        BNF.DATINGCONBEN,
                        BNF.DATSITBEN,
                        BNF.DATALTBEN,
                        PAC.CAD_PAC_ID_PACIENTE,
                        DECODE(FNC_VERIFICA_INSGER(BNF.CODCON,BNF.CODEST,BNF.CODBEN,BNF.CODSEQBEN),0,'||CHR(39)||''||CHR(39)||',1,'||CHR(39)||'I.GER'||CHR(39)||',2,'||CHR(39)||'I.GER'||CHR(39)||',4,'||CHR(39)||'A.ALTA'||CHR(39)||') GER
                   ...
                   JOIN BNF_BENEFICIARIO       BNF ON TO_CHAR(BNF.CODCON) =  TO_CHAR(PLA.CAD_PLA_CD_PLANO_HAC)
                        AND LPAD(BNF.CODEST,3,'||CHR(39)||'0'||CHR(39)||')||LPAD(BNF.CODBEN,7,'||CHR(39)||'0'||CHR(39)||')||LPAD(BNF.CODSEQBEN,2,'||CHR(39)||'0'||CHR(39)||') =  PAC.CAD_PAC_CD_CREDENCIAL
      ...
      JOIN      TB_ATD_INA_INT_ALTA        INA   ON INA.ATD_ATE_ID                 = ATD.ATD_ATE_ID
            AND (NOT EXISTS
                  (SELECT GUI.ATD_ATE_ID
                     FROM TB_ATD_GUI_GUIAATEND GUI
                    WHERE GUI.ATD_ATE_ID = ATD.ATD_ATE_ID) OR
                  (EXISTS
                   (SELECT GUI.ATD_ATE_ID
                      FROM TB_ATD_GUI_GUIAATEND GUI
                     WHERE GUI.ATD_ATE_ID = ATD.ATD_ATE_ID) AND
                   NOT EXISTS
                   (SELECT GUI.ATD_ATE_ID
                      FROM TB_ATD_GUI_GUIAATEND GUI
                     WHERE GUI.ATD_ATE_ID              = ATD.ATD_ATE_ID
                       AND GUI.CAD_PAC_ID_PACIENTE     = PAC.CAD_PAC_ID_PACIENTE
                       AND GUI.ATD_GUI_FL_GUIAPRINC_OK = '||CHR(39)||'S'||CHR(39)||'
                       AND GUI.ATD_GUI_FL_ATIVO_OK     = '||CHR(39)||'S'||CHR(39)||')
                       )
                 )
     ...
     WHERE
           (AIC.ATD_AIC_TP_SITUACAO_PAC = '||CHR(39)||'A'||CHR(39)||')
       AND (ATD.ATD_ATE_FL_STATUS = '||CHR(39)||'A'||CHR(39)||')

       AND ('||CHR(39)|| pCAD_CGC_ID_G ||CHR(39)|| 'IS NOT NULL AND CNV.CAD_CGC_ID = 1 OR '||CHR(39)|| pCAD_CGC_ID_E ||CHR(39)|| 'IS NOT NULL AND CNV.CAD_CGC_ID = 2)
...
OPEN v_cursor FOR
  -- select * from dual;
   V_SELECT ;
    io_cursor := v_cursor;
end PRC_INT_REL_SAIDA_SEM_PRINCP;