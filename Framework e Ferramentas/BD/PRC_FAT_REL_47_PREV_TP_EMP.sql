CREATE OR REPLACE PROCEDURE SGS."PRC_FAT_REL_47_PREV_TP_EMP"
(
...
IS
/********************************************************************
*    Procedure: PRC_FAT_REL_47_PREV_TP_EMP
*     FAT_47 parecida com PRC_FAT_REL_MOV_CONTA_PREVIA
*
*    Data Alteracao: 5/9/2013  Por: Pedro
*    Altera??o: custo
*
*
*******************************************************************/
...
'
 SELECT DISTINCT
       ...
       CAD_UNI_ID_UNIDADE,
       CAD_UNI_DS_UNIDADE,
       CASE WHEN TIPO_PLANO IN ('||chr(39)||'SERVICO PRESTADO'||chr(39)||') THEN
             SUM(FATURADOS) OVER (PARTITION BY TIPO_PLANO,DESPESA_RECEITA,LOCAL_ATENDIMENTO,CAD_LAT_DS_LOCAL_ATENDIMENTO, CAD_UNI_ID_UNIDADE, CAD_UNI_DS_UNIDADE) +
             SUM(FATURADOS_NOF) OVER (PARTITION BY TIPO_PLANO,DESPESA_RECEITA,LOCAL_ATENDIMENTO,CAD_LAT_DS_LOCAL_ATENDIMENTO, CAD_UNI_ID_UNIDADE, CAD_UNI_DS_UNIDADE)
            ELSE
              SUM(FATURADOS) OVER (PARTITION BY TIPO_PLANO,DESPESA_RECEITA,LOCAL_ATENDIMENTO,CAD_LAT_DS_LOCAL_ATENDIMENTO, CAD_UNI_ID_UNIDADE, CAD_UNI_DS_UNIDADE) +
              SUM(FATURADOS_NOF) OVER (PARTITION BY TIPO_PLANO,DESPESA_RECEITA,LOCAL_ATENDIMENTO,CAD_LAT_DS_LOCAL_ATENDIMENTO, CAD_UNI_ID_UNIDADE, CAD_UNI_DS_UNIDADE) +
              SUM(EMITIDOS) OVER (PARTITION BY TIPO_PLANO,DESPESA_RECEITA,LOCAL_ATENDIMENTO,CAD_LAT_DS_LOCAL_ATENDIMENTO, CAD_UNI_ID_UNIDADE, CAD_UNI_DS_UNIDADE)
       END PROCESSADO,
       ...
       CASE WHEN TIPO_PLANO IN ('||chr(39)||'SERVICO PRESTADO'||chr(39)||') THEN
            ...
            ELSE
              ...
              SUM(EMITIDOS) OVER (PARTITION BY TIPO_PLANO,DESPESA_RECEITA,LOCAL_ATENDIMENTO,CAD_LAT_DS_LOCAL_ATENDIMENTO, CAD_UNI_ID_UNIDADE, CAD_UNI_DS_UNIDADE)
       END +
       SUM(DIGITADOS) OVER (PARTITION BY TIPO_PLANO,DESPESA_RECEITA,LOCAL_ATENDIMENTO,CAD_LAT_DS_LOCAL_ATENDIMENTO, CAD_UNI_ID_UNIDADE, CAD_UNI_DS_UNIDADE) +
       SUM(AUDITORIA) OVER (PARTITION BY TIPO_PLANO,DESPESA_RECEITA,LOCAL_ATENDIMENTO,CAD_LAT_DS_LOCAL_ATENDIMENTO, CAD_UNI_ID_UNIDADE, CAD_UNI_DS_UNIDADE)
      TOTAL
 FROM
  (
    SELECT
           CASE WHEN TP_PLA.CAD_TPE_CD_CODIGO IN ('||chr(39)||'ACS'||chr(39)||') THEN '||chr(39)||'ANA COSTA SAUDE'||chr(39)||'
                 WHEN TP_PLA.CAD_TPE_CD_CODIGO IN ('||chr(39)||'SP'||chr(39)||')  THEN '||chr(39)||'SERVICO PRESTADO'||chr(39)||'
                 WHEN TP_PLA.CAD_TPE_CD_CODIGO IN ('||chr(39)||'PA'||chr(39)||')  THEN '||chr(39)||'PARTICULAR'||chr(39)||'
                 WHEN TP_PLA.CAD_TPE_CD_CODIGO IN ('||chr(39)||'FU'||chr(39)||')  THEN '||chr(39)||'FUNCIONARIO HAC'||chr(39)||'
                 WHEN TP_PLA.CAD_TPE_CD_CODIGO IN ('||chr(39)||'FU-S077'||chr(39)||')  THEN '||chr(39)||'FUNCIONARIO ACS'||chr(39)||'
            ...
            UNI.CAD_UNI_ID_UNIDADE,
             CASE WHEN UNI.CAD_UNI_ID_UNIDADE = 244 AND DIFERENCA.DIF = '||chr(39)||'COMDIF'||chr(39)||' AND TP_PLA.CAD_TPE_CD_CODIGO = '||chr(39)||'PA'||chr(39)||' THEN '||chr(39)||'DIF. DE CLASSE - SANTOS'||chr(39)||'
                 ...
                 WHEN UNI.CAD_UNI_ID_UNIDADE = 245 AND DIFERENCA.DIF = '||chr(39)||'COMDIF'||chr(39)||' AND TP_PLA.CAD_TPE_CD_CODIGO = '||chr(39)||'PA'||chr(39)||' THEN '||chr(39)||'DIF. DE CLASSE - GUARUJ?'||chr(39)||'
    ...
    LEFT JOIN (  SELECT  SUM(CCI.FAT_CCI_VL_FATURADO) TOTAL,
                        ...
                        CASE WHEN PLA.CAD_PLA_CD_PLANO_HAC = ''S077'' THEN ''FU-S077'' ELSE CNV.CAD_TPE_CD_CODIGO END CAD_TPE_CD_CODIGO,
             ...
             ' || V_WHERE || '
              ...
              GROUP BY
                      CCI.CAD_UNI_ID_UNIDADE,
                      CCI.CAD_LAT_ID_LOCAL_ATENDIMENTO,
                      CASE WHEN PLA.CAD_PLA_CD_PLANO_HAC = ''S077'' THEN ''FU-S077'' ELSE CNV.CAD_TPE_CD_CODIGO END,
                     CASE WHEN CCP.FAT_NOF_ID IS NOT NULL THEN '||CHR(39)||'FATURADO'||CHR(39)||'
                 WHEN CCP.FAT_CCP_FL_STATUS_AUDIT = '||CHR(39)||'E'||CHR(39)||' THEN '||CHR(39)||'AUDITORIA'||CHR(39)||'
                 WHEN CCP.FAT_CCP_FL_FATURADA = '||CHR(39)||'S'||CHR(39)||' AND CCP.FAT_NOF_ID IS NULL THEN '||CHR(39)||'LOTE'||CHR(39)||'
                 WHEN CCP.FAT_CCP_FL_EMITIDA = '||CHR(39)||'S'||CHR(39)||' AND CCP.FAT_CCP_FL_FATURADA = '||CHR(39)||'N'||CHR(39)||'
 ...
 --  TESTE :=  V_SELECT ;
  ...
  OPEN v_cursor FOR
   V_SELECT ;
    io_cursor := v_cursor;
END PRC_FAT_REL_47_PREV_TP_EMP;
/
