create or replace function FNC_AGE_ESM_TIPO_NORMAL
(
...
begin
       ...
       AND AGG.AGE_AGG_TP_HORARIO = 1
       AND AGG.AGE_AGG_FL_STATUS_HORARIO = 2       
       AND AGG.AGE_AGG_DT_AGENDA BETWEEN pDATA_INI AND pDATA_FIM
       AND AGG.AGE_AGG_ID NOT IN (SELECT AGE.AGE_AGG_ID FROM  TB_AGE_AGD_AGENDA AGE WHERE  AGE.AGE_ESM_ID = pAGE_ESM_ID
            AND (AGE.Age_Agd_Fl_Telemedicina = 'S'))
       ;
  return(Result);
end FNC_AGE_ESM_TIPO_NORMAL;
